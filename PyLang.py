...
class List(Value):
    ...
    def append(self, other : Value) -> tuple[List, RTError]:
        new_list = self.copy()
    ...
    def delete(self, other : Number) -> tuple[List, RTError]:
        if isinstance(other, Number):
            new_list = self.copy()
            try:
    ...
    def get(self, other : Number) -> tuple[Value, RTError]:
        if isinstance(other, Number):
            try:
    ...
    def execute(self, args : list[Value]) -> RTResult:
        ...
        if len(args) > len(self.arg_names):
            return result.failure(RTError(
                self.pos_start, self.pos_end,
                f"Too many args passed into '{self.name}'!\nNeeded {len(self.arg_names)}, given {len(args)}",
                self.context
            ))
